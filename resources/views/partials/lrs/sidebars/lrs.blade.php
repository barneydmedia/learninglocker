  ...
  <ul class="nav nav-sidebar">
    <li class="link">
      <select class="form-control sidebar-select" onchange="javascript:location.href = this.value;">
          <option></option>
        <optgroup label="List">
          <option value="/site#lrs">{{ Lang::get('lrs.home') }}</option>
        </optgroup>
        <optgroup label="Available LRSs">
          @if( isset($list) )
            @foreach( $list as $l )
              <option value="/lrs/{{ $l->_id }}" @if($l->_id == $lrs->_id) SELECTED @endif>{{ $l->title }}</option>
            @endforeach
          @endif
        </optgroup>
      </select>
